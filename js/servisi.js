	...
	function ubaciRezultate(result)
	{
		...
		else 
		{
			for (i=0; i < res.length-1; i++)
			{
				//ubaci rezultate
				var html =
				'<div class="col-md-4 text-center hranaBox">'+
	                '<img src="img/'+ res[i].slika +'" alt="Hrana!" class="img-thumbnail">'+
	                '<p class="text-danger"></p>'+
	                '<div class="kat">Kategorija: '+res[i].kategorija+'</div>'+
	                '<h3>'+res[i].naziv+'</h3>'+
	                '<p><span class="text-primary">Sastojci:</span> '+res[i].sastojci.join(", ")+'</p>'+
	                '<p class="text-success">'+res[i].cena+' RSD</p>'+
	                '<a class="btn btn-primary dodaj" href="#" data-id="'+res[i]._id.$id+'">Dodaj u Korpu</a>'+
	            '</div>';
